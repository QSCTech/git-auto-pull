...
static64:
	mkdir -p tmp
	$(CC) -static src/main.c -D VERSION=\"$(VERSION)\" $(CFLAGS) -Wl,--as-needed -Wa,--noexecstack -I $(LIBDIR)/http-parser-bin64/ -L $(LIBDIR)/http-parser-bin64/ -I $(LIBDIR)/libssh2-bin64/include -L $(LIBDIR)/libgit2-bin64 -L $(LIBDIR)/json-c-bin64/lib -I $(LIBDIR)/libgcrypt-bin64/include -I $(LIBDIR)/libgit2/include -I $(LIBDIR)/json-c-bin64/include $(LIBDIR)/libssh2-bin64/lib/libssh2.a $(LIBDIR)/http-parser-bin64/libhttp_parser.a $(LIBDIR)/libgit2-bin64/libgit2.a $(LIBDIR)/json-c-bin64/lib/libjson-c.a -o tmp/git-auto-pull-static_amd64 -L $(LIBDIR)/libssh2-bin64/lib -lssh2 -L $(LIBDIR)/libgcrypt-bin64/lib -l gcrypt -L $(LIBDIR)/libgpg-error-bin64/lib -I $(LIBDIR)/openssl-bin64/include -L $(LIBDIR)/zlib-bin64/lib -lz -L $(LIBDIR)/openssl-bin64/lib -lssl -lcrypto $(LIBDIR)/openssl-bin64/lib/libssl.a $(LIBDIR)/openssl-bin64/lib/libcrypto.a -L /usr/lib/z86_64-linux-gnu/ -lrt -ldl -m64

static32:
	mkdir -p tmp
	$(CC) -static src/main.c -D VERSION=\"$(VERSION)\" -Wl,--as-needed -Wa,--noexecstack -I $(LIBDIR)/http-parser-bin32/ -L $(LIBDIR)/http-parser-bin32/ -I $(LIBDIR)/libssh2-bin32/include -L $(LIBDIR)/libgit2-bin32 -L $(LIBDIR)/json-c-bin32/lib -I $(LIBDIR)/libgcrypt-bin32/include -I $(LIBDIR)/libgit2/include -I $(LIBDIR)/json-c-bin32/include $(LIBDIR)/libssh2-bin32/lib/libssh2.a $(LIBDIR)/http-parser-bin32/libhttp_parser.a $(LIBDIR)/libgit2-bin32/libgit2.a $(LIBDIR)/json-c-bin32/lib/libjson-c.a -o tmp/git-auto-pull-static_i386 -L $(LIBDIR)/libssh2-bin32/lib -lssh2 -L $(LIBDIR)/libgcrypt-bin32/lib -L $(LIBDIR)/libgpg-error-bin32/lib -I $(LIBDIR)/openssl-bin32/include -L $(LIBDIR)/zlib-bin32/lib -lz -L $(LIBDIR)/openssl-bin32/lib -lssl -lcrypto $(LIBDIR)/openssl-bin32/lib/libssl.a $(LIBDIR)/openssl-bin32/lib/libcrypto.a -L /usr/lib/i386-linux-gnu/ -lrt -ldl -m32
